...
class MessageAdapter : PagedListAdapter<Discuss, DiscussViewHolder>(diffCallback) {
...
class DiscussViewHolder(parent: ViewGroup) : RecyclerView.ViewHolder(
    ...
    fun bind(item: Discuss?) {
        item?.let {
            tvDiscussTitle.text = it.discussTitle

            itemView.setOnClickListener {
                context.startKtxActivity<DiscussMessageActivity>(value = DiscussMessageActivity.KEY_DISCUSS_ID to it.id)
            }
        }
    }
